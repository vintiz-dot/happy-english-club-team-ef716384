-- 1. Add the celebration_seen_at column if it does not exist
ALTER TABLE public.homework_submissions
  ADD COLUMN IF NOT EXISTS celebration_seen_at TIMESTAMPTZ;

-- 2. Create an index for faster lookups on pending celebrations
...
-- 3. Update the get_student_homeworks RPC to include celebration_seen_at in the output
  ...
  SELECT jsonb_build_object(
    ...
    'submissions', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'celebration_seen_at', hs.celebration_seen_at
      ))
